  ...
  for (let i = 0; i < mainList.length; i++) {
    if (filterVar == "music") {
      if (mainList[i][2] == "music") {
        ...
        list +=
          "<div id='edit'> <button onclick='editItem(" +
          i +
        ...
        list +=
          "<span id='edit-box" +
          i +
          "' role='aria-hidden' class='edit-box'> <form action='javascript:void(0);' method='post' id='save-edit" +
          i +
          "'> <input type='text' class='edit" +
          i +
          "' id='edit-page'> <input type='submit' value='save' class='save-button'> <a onclick='CloseInput(" +
          i +
          ")' aria-label='close'> \u00D7 </a> </form> </span> </div>";
...
function editItem(index) {
  ...
  document.getElementById(forText).onsubmit = function () {
    ...
    for (let i = 0; i < mainList.length; i++) {
      let edit_data =
        mainList[i][0] + "," + mainList[i][1] + "," + mainList[i][2];
      var old_data = JSON.parse(myStorage.getItem("data"));
...
function storageLoad() {
  if (myStorage.getItem("data") != null) {
    ...
    for (let i = 0; i < rawList.length; i += 3)
      mainList.push(rawList.slice(i, i + 3));
  }
}
